...
<body style="font-size: 11px;">
    ...
    <table border="0" width="50%">
        ...
        $sql = mysqli_query($koneksi, "SELECT * FROM member where month(tgl_daf)='$bln' and year(tgl_daf)='$thn'"); // Eksekusi/Jalankan query dari variabel $query
        ...
        if ($row > 0) { // Jika jumlah data lebih dari 0 (Berarti jika data ada)
            ...
            while ($data = mysqli_fetch_array($sql)) { // Ambil semua data dari hasil eksekusi $sql
                ...
                echo "<td  width='80'>" . $data['p_no_identitas'] . "</td>";
                echo "<td width='100'>" . $data['p_nama'] . "</td>";
                echo "<td width='150'>" . $data['email'] . "</td>";
                echo "<td  width='80'>" . $data['p_nohp'] . "</td>";
                echo "<td  width='120'>" . $data['p_alamat'] . "</td>";
                echo "</tr>";
            }
...
$pdf->WriteHTML($html);
$pdf->Output('laporan_member.pdf', 'D');
?>